...
def init_model(device):
    ...
    model = VisionEncoderDecoderModel.from_pretrained(decoder_model).to(device)
...
def init_dataloader(batch_size=5):
    ...
    def transform_caption(caption: str) -> str:
        for s in '.,!?:;':
    ...
    # captions = list(map(transform_caption, captions))
...
def test_picture(device, model, tokenizer, image_processor, file_path, verbose=False):
    model.eval()
    ...
    if verbose:
        generated_text_list = tokenizer.batch_decode(generated_ids, skip_special_tokens=False)
...
if __name__ == '__main__':
    ...
    model, tokenizer, image_processor = init_model_offline(device)
    ...
    generated_text = test_picture(device, model, tokenizer, image_processor, test_file, verbose=False)
